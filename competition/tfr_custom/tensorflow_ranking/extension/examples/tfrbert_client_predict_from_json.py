...
class TFRBertUtilJSON(object):
    ...
    def convert_json_to_elwc(self, filenameQueryJsonIn, query_key, filenameDocJsonIn, filenameJsonOut):
        ...
        try:
            with open(filenameQueryJsonIn) as query_file, open(filenameDocJsonIn) as doc_file:
                ...
                num_runs = len(range(0, len(uid_list), docs_at_once))
                ...
                for q_idx, query in queries.items():
                    ...
                    for d_uid in uid_list:
                        ...
                        if doc_count > docs_at_once or d_uid == last_doc:
                            pred_count += 1
                            percentCompleteStr = "{:.2f}".format(float(pred_count) * 100 / float(num_runs))
                            print("Query " + q_idx + " Predicting " + str(pred_count) + " / " + str(num_runs) + " (" + percentCompleteStr + "%)")
